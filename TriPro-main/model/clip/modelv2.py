...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        self.cross_attn_attr = nn.MultiheadAttention(d_model, n_head)
        self.ln_attr = LayerNorm(d_model)
    def attention(self, x: torch.Tensor):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]

    def cross_attention(self, x: torch.Tensor, attr_embeds: torch.Tensor):
        return self.cross_attn_attr(x, attr_embeds, attr_embeds, need_weights=False, attn_mask=None)[0]

    def forward(self, x: torch.Tensor, attr_embeds: torch.Tensor=None):
        x = x + self.attention(self.ln_1(x))
        if attr_embeds is not None:
            attr_embeds = attr_embeds.permute(1,0,2)
            x = x + self.cross_attention(self.ln_1(x), self.ln_attr(attr_embeds))
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cv_emb = None, attr_embeds=None):
        ...
        x11 = x
        for blk in self.transformer.resblocks[:11]:
            x11 = blk(x11, attr_embeds) 

        x12 = self.transformer.resblocks[11](x11, attr_embeds) 
