...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, prompt: torch.Tensor = None, visual_mask: torch.Tensor = None):
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cv_emb = None, attr_embeds=None, visual_prompt=None, temporal_prompt=None, return_attention=False):
        ...
        if attr_embeds is not None:
            N = attr_embeds.shape[1]
            x = torch.cat([x[:,:1], attr_embeds, x[:, 1:]],dim=1)
        ...
        if temporal_prompt is not None:
            T, tpl, D = temporal_prompt.shape
            x11 = torch.cat([x11, temporal_prompt.unsqueeze(1).repeat(1,B//T,1,1).view(tpl, -1, D)],dim=0)
        if visual_prompt is not None:
            depth = visual_prompt.shape[0]
            pl = visual_prompt.shape[1]
            x11 = torch.cat([x11[:1], visual_prompt[0].unsqueeze(1).repeat(1, B, 1), x11[1:]])
        for bid, blk in enumerate(self.transformer.resblocks[:11]):
            if visual_prompt is not None and bid<depth:
                x11 = torch.cat([x11[:1], visual_prompt[bid].unsqueeze(1).repeat(1, B, 1), x11[1+pl:]],dim=0)
            x11, attn = blk(x11) 
        if visual_prompt is not None and depth == 12:
            x11 = torch.cat([x11[:1], visual_prompt[-1].unsqueeze(1).repeat(1, B, 1), x11[1+pl:]])
        # x11 = self.transformer.resblocks[:11](x) 
        x12, attn = self.transformer.resblocks[11](x11) 
        # x11 = torch.cat([x11[:1], x11[1+pl:-tpl]])
        # x12 = torch.cat([x12[:1], x12[1+pl:-tpl]])
