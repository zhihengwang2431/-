...
class PadResize224(object):
    """
    Pad each frame to square with black border, then resize to 224x224.
    Input: list of lists of PIL images: [[frame0, frame1, ...]]
    """

    def __init__(self, target_size=224, interpolation=Image.BILINEAR):
        self.target_size = target_size
        self.interpolation = interpolation

    def __call__(self, seqs):  # seqs = list[[image0,...,imageN]]
        modallen = len(seqs)  # 1个modal，例如RGB
        framelen = len(seqs[0])  # 比如 8 帧
        ...
        for modal_ind, modal in enumerate(seqs):
            for frame_ind, frame in enumerate(modal):
                ...
                # Step 1: Pad to square (longer edge)
                max_side = max(w, h)
                ...
                padded = Image.new("RGB", (max_side, max_side), (0, 0, 0))  # 黑边填充
                padded.paste(frame, (pad_left, pad_top))

                # Step 2: Resize to target size
                resized = padded.resize((self.target_size, self.target_size), self.interpolation)
                new_seqs[modal_ind][frame_ind] = resized
