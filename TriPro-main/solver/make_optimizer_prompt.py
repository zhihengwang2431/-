...
def make_optimizer_2stage(cfg, model, center_criterion):
    ...
    for key, value in model.named_parameters():
        ...
        if "prompt_learner" in key and 'attr' not in key:
            value.requires_grad_(False)
            continue
        ...
        params += [{"params": [value], "lr": lr, "weight_decay": weight_decay}]
        keys += [key]
        print(f'Learnable Params [{key}]----{value.shape}')
...
def make_optimizer_mid_stage(cfg, model):
    params = []
    keys = []
    for key, value in model.named_parameters():
        if "rgb_prompt" in key or "eve_prompt" in key or 'temporal_prompt' in key:
            lr = cfg.SOLVER.STAGE1.BASE_LR
            weight_decay = cfg.SOLVER.STAGE1.WEIGHT_DECAY
            params += [{"params": [value], "lr": lr, "weight_decay": weight_decay}]
            keys += [key]
